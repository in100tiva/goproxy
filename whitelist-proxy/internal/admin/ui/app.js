...
function detectRuleType(pattern) {
  ...
  // Heurística regex: presença de metacaracteres não usuais em hostnames.
...
function openAddModal({ initial = "" } = {}) {
  ...
  const updateDetection = () => {
    const t = detectRuleType($pattern.value);
    if (t) {
      $type.value = t;
    ...
    if ($type.value === "regex" && $pattern.value.trim()) {
      try { new RegExp($pattern.value); }
...
async function submitAddModal() {
  ...
  state.rules.push({ _id: newId(), pattern, type, note });
  closeModal();
  ...
  try {
    ...
    await api("/api/whitelist", { method: "PUT", body: JSON.stringify({ rules: cleaned }) });
    state.dirty = false;
    updateDirtyUI();
    refreshRules();
    refreshDashboard(); // atualiza badge e contador
    toast({ type: "success", title: "Regra adicionada", message: `"${pattern}" agora é permitido.` });
  } catch (err) {
